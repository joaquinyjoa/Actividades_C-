...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        for (int i = 0; i < paquetes.Count; i++)
        {
            ...
            {
                stringBuilder.AppendLine($"Impuesto AFIP: ${paqueteAfip.Impuestos:#0.00}");
            }

            if (paquetes[i] is IAduana paqueteAduana)
            {
                stringBuilder.AppendLine($"Impuesto aduana: ${paqueteAduana.Impuestos:#0.00}");
            }

            stringBuilder.AppendLine($"Costo de envío con impuestos: ${paquetes[i].AplicarImpuestos():#0.00}");
        }
